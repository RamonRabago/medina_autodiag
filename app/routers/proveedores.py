...
def listar_proveedores(
    ...
    if buscar and buscar.strip():
        term = f"%{buscar.strip()}%"
        query = query.filter(
            (Proveedor.nombre.like(term)) | (Proveedor.telefono.like(term)) | (Proveedor.email.like(term))
        )
    total = query.count()
...
def obtener_proveedor(
    id_proveedor: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    """
    ...
    if not proveedor:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Proveedor con ID {id_proveedor} no encontrado"
        )
...
def eliminar_proveedor(
    id_proveedor: int, db: Session = Depends(get_db), current_user: Usuario = Depends(require_roles("ADMIN"))
):
    """
...
def reactivar_proveedor(
    ...
    if proveedor.activo:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"El proveedor '{proveedor.nombre}' ya está activo"
        )
