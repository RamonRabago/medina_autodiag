...
from app.schemas.categoria_repuesto import CategoriaRepuestoCreate, CategoriaRepuestoOut, CategoriaRepuestoUpdate
from app.utils.dependencies import get_current_user
from app.utils.roles import require_roles

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/categorias-repuestos", tags=["Inventario - Categorías"])
...
def crear_categoria(
    ...
    categoria_existente = db.query(CategoriaRepuesto).filter(CategoriaRepuesto.nombre == categoria.nombre).first()
...
def obtener_categoria(
    id_categoria: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    """
    Obtiene una categoría específica por ID.
    """
    categoria = db.query(CategoriaRepuesto).filter(CategoriaRepuesto.id_categoria == id_categoria).first()

    if not categoria:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Categoría con ID {id_categoria} no encontrada"
        )
...
def actualizar_categoria(
    ...
    if categoria_update.nombre and categoria_update.nombre != categoria.nombre:
        nombre_existente = (
            db.query(CategoriaRepuesto).filter(CategoriaRepuesto.nombre == categoria_update.nombre).first()
        )
...
def eliminar_categoria(
    id_categoria: int, db: Session = Depends(get_db), current_user: Usuario = Depends(require_roles("ADMIN"))
):
    """
