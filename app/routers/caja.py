...
def abrir_turno(data: TurnoAbrir, db: Session = Depends(get_db), current_user=Depends(require_roles("ADMIN", "CAJA"))):
    turno_abierto = (
        db.query(CajaTurno)
        .filter(CajaTurno.id_usuario == current_user.id_usuario, CajaTurno.estado == "ABIERTO")
        .first()
    )
    ...
    turno = CajaTurno(id_usuario=current_user.id_usuario, monto_apertura=data.monto_apertura, estado="ABIERTO")
...
def cerrar_turno(
    data: TurnoCerrar, db: Session = Depends(get_db), current_user=Depends(require_roles("ADMIN", "CAJA"))
):
    turno = (
        db.query(CajaTurno)
        .filter(CajaTurno.id_usuario == current_user.id_usuario, CajaTurno.estado == "ABIERTO")
    ...
    try:
        turno = cerrar_turno_service(db=db, id_turno=turno.id_turno, monto_contado=data.monto_cierre)
        registrar_auditoria(
            db,
            current_user.id_usuario,
            "CERRAR",
            "CAJA_TURNO",
            turno.id_turno,
            {"monto_cierre": float(data.monto_cierre)},
        )
        return turno
...
def turno_actual(db: Session = Depends(get_db), current_user=Depends(require_roles("ADMIN", "CAJA"))):
    turno = (
        db.query(CajaTurno)
        .filter(CajaTurno.id_usuario == current_user.id_usuario, CajaTurno.estado == "ABIERTO")
        .first()
...
def corte_diario(
    fecha: date | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles("ADMIN", "CAJA"))
):
    if not fecha:
        fecha = date.today()
    ...
    total_general = db.query(func.coalesce(func.sum(Pago.monto), 0)).filter(Pago.id_turno == turno.id_turno).scalar()
    ...
    return {
        ...
        "totales_por_metodo": [{"metodo": metodo, "total": float(total)} for metodo, total in resultados],
...
def detalle_turno(id_turno: int, db: Session = Depends(get_db), current_user=Depends(require_roles("ADMIN", "CAJA"))):
    ...
    efectivo_esperado = float(turno.monto_apertura or 0) + efectivo_ingresos - total_pagos_proveedores - total_gastos
    ...
    return {
        ...
        "totales_por_metodo": [{"metodo": metodo, "total": float(total)} for metodo, total in totales],
...
def cerrar_turno_forzado(
    ...
    try:
        turno = cerrar_turno_service(db=db, id_turno=turno.id_turno, monto_contado=Decimal(str(monto_cierre)))
        registrar_auditoria(
            db,
            current_user.id_usuario,
            "CERRAR_FORZADO",
            "CAJA_TURNO",
            turno.id_turno,
            {"monto_cierre": float(monto_cierre), "motivo": motivo or "Cierre forzado por ADMIN"},
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
# ======================================================
@router.get("/alertas")
def alertas_cajero(db: Session = Depends(get_db), current_user=Depends(require_roles("CAJA", "ADMIN"))):
    ...
    alertas = db.query(CajaAlerta).filter(CajaAlerta.id_turno == turno.id_turno, not CajaAlerta.resuelta).all()

    # 3️⃣ Respuesta simple para UI
    return [{"id_alerta": a.id_alerta, "tipo": a.tipo, "nivel": a.nivel, "mensaje": a.mensaje} for a in alertas]
