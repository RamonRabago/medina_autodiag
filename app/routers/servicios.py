...
from app.schemas.servicio_schema import ServicioCreate, ServicioListResponse, ServicioResponse, ServicioUpdate
...
def listar_servicios(
    ...
    if buscar:
        ...
        query = query.filter((Servicio.codigo.like(buscar_pattern)) | (Servicio.nombre.like(buscar_pattern)))
...
def obtener_servicio(
    servicio_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    """
    ...
    if not servicio:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Servicio con ID {servicio_id} no encontrado"
        )
...
def actualizar_servicio(
    ...
    if servicio_data.codigo and servicio_data.codigo != servicio.codigo:
        codigo_existente = (
            db.query(Servicio).filter(Servicio.codigo == servicio_data.codigo, Servicio.id != servicio_id).first()
        )
        if codigo_existente:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
def eliminar_servicio(
    servicio_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(require_roles(["ADMIN"]))
):
    """
...
def listar_categorias_legacy(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    """
    ...
    """
    cats = db.query(CategoriaServicio).filter(CategoriaServicio.activo).order_by(CategoriaServicio.nombre).all()
