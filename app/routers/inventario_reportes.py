...
from app.schemas.alerta_inventario import AlertaInventarioOut, ResumenAlertas
...
router = APIRouter(prefix="/inventario", tags=["Inventario - Reportes"])
        ...
        grupos.append(
            {
                ...
                "items": sin_proveedor,
                "total_estimado": round(total_sin, 2),
            }
        )
...
def listar_alertas(
    ...
    query = (
        ...
        .filter(not Repuesto.eliminado)
    )

    if activas_solo:
        query = query.filter(AlertaInventario.activa)

    if tipo_alerta:
        query = query.filter(AlertaInventario.tipo_alerta == tipo_alerta)

    alertas = (
        query.options(joinedload(AlertaInventario.repuesto))
...
def resumen_alertas(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    """
...
def resolver_alerta(
    id_alerta: int, db: Session = Depends(get_db), current_user: Usuario = Depends(require_roles("ADMIN", "CAJA"))
):
    """
    ...
    Requiere rol: ADMIN o CAJA
    """
    alerta = db.query(AlertaInventario).filter(AlertaInventario.id_alerta == id_alerta).first()

    if not alerta:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Alerta con ID {id_alerta} no encontrada")

    if not alerta.activa:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="La alerta ya está resuelta")
    ...
    return {"mensaje": "Alerta resuelta exitosamente", "id_alerta": id_alerta}
...
def verificar_productos_sin_movimiento(
    ...
    return {"mensaje": f"Verificación completada para productos sin movimiento en {dias} días"}
...
def reporte_valor_inventario(
    db: Session = Depends(get_db), current_user: Usuario = Depends(require_roles("ADMIN", "CAJA"))
):
    """
    ...
    return {"fecha_reporte": datetime.utcnow().isoformat(), **valor}
...
def reporte_productos_mas_vendidos(
    ...
    return {"fecha_reporte": datetime.utcnow().isoformat(), "productos": productos}


@router.get("/reportes/stock-bajo")
def reporte_stock_bajo(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    """
    Lista todos los productos con stock bajo o crítico.
    """
    productos_stock_bajo = (
        db.query(Repuesto)
        .filter(Repuesto.activo, not Repuesto.eliminado, Repuesto.stock_actual <= Repuesto.stock_minimo)
        .order_by(Repuesto.stock_actual.asc())
        .all()
    )
    ...
    return {"fecha_reporte": datetime.utcnow().isoformat(), "total_productos": len(resultado), "productos": resultado}
...
def reporte_rotacion_inventario(
    ...
    ventas = (
        ...
        .join(MovimientoInventario, Repuesto.id_repuesto == MovimientoInventario.id_repuesto)
        .filter(
    ...
    for venta in ventas:
        if venta.stock_actual > 0:
            rotacion = float(
                (venta.cantidad_vendida / venta.stock_actual) * (Decimal(30) / Decimal(dias))
            )  # Normalizado a 30 días
        else:
            rotacion = 0
...
def dashboard_inventario(
    ...
    total_productos = (
        db.query(func.count(Repuesto.id_repuesto)).filter(Repuesto.activo, not Repuesto.eliminado).scalar()
    )
