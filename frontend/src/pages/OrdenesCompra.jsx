...
export default function OrdenesCompra() {
  ...
  const { data: dataOrdenes, isLoading: loadingOrdenes, refetch } = useApiQuery(
    ['ordenes-compra', pagina, filtroEstado, filtroProveedor],
  ...
  const enviarOrden = async (oc) => {
    try {
      await api.post(`/ordenes-compra/${oc.id_orden_compra}/enviar`)
  ...
  const colorEstado = (est) => {
    const m = { BORRADOR: 'bg-slate-100 text-slate-700', ENVIADA: 'bg-blue-100 text-blue-800', RECIBIDA: 'bg-green-100 text-green-800', RECIBIDA_PARCIAL: 'bg-amber-100 text-amber-800', CANCELADA: 'bg-red-100 text-red-700' }
                ...
                ordenes.map((o) => (
                  <tr key={o.id_orden_compra} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-xs ${colorEstado(o.estado)}`}>{o.estado}</span>
                    </td>
        ...
        {cargandoDetalle ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              <div><span className="text-slate-500">Estado:</span> <span className={`px-2 py-0.5 rounded text-xs ${colorEstado(ordenDetalle.estado)}`}>{ordenDetalle.estado}</span></div>
            ...
            {puedeGestionar && (
              <div className="flex gap-2 flex-wrap pt-2 border-t">
                {ordenDetalle.estado === 'BORRADOR' && (
                  <button onClick={() => enviarOrden(ordenDetalle)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm">Enviar orden</button>
                )}
                ...
                {(ordenDetalle.estado === 'BORRADOR' || ordenDetalle.estado === 'ENVIADA') && user?.rol === 'ADMIN' && (
